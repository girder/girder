...
def serverContext(plugins=None, bindPort=False):
    ...
    if bindPort:
        # As of cheroot 11.0.0, the cheroot server creates a non-daemon thread
        # that prevents pytest from shutting down in some instances.  Make the
        # code within that thread just return None so that it doesn't stop
        # pytest from stopping.
        try:
            import cheroot
