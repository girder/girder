...
class Resource(BaseResource):
    ...
    @describeRoute(
        ...
        .param('mode', 'The search mode. Can use either a text search or a '
               'prefix-based search.', enum=('text', 'prefix'), required=False, default='text')
    ...
    def search(self, params):
        ...
        if mode == 'text':
            ...
            raise RestException('The search mode must be either "text" or "prefix".')
        ...
        for modelName in types:
            ...
            results[modelName] = [
                model.filter(d, user) for d in getattr(model, method)(
                    query=params['q'], user=user, limit=limit, offset=offset, level=level)
            ]
    ...
    def _validateResourceSet(self, params, allowedModels=None):
        ...
        :param allowedModels: if present, an iterable of models that may be included in the
            resources.
    ...
    @describeRoute(
        ...
        .param('path',
               'The path of the resource.  The path must be an absolute Unix path starting with '
               'either "/user/[user name]", for a user\'s resources or '
               '"/collection/[collection name]", for resources under a collection.')
        .param('test',
               'Specify whether to return None instead of throwing an exception when path doesn\'t '
               'exist.', required=False, dataType='boolean', default=False)
    ...
    @access.public(scope=TokenScope.DATA_READ)
    @describeRoute(
        Description('Download a set of items, folders, collections, and users as a zip archive.')
        ...
        .param('includeMetadata', 'Include any metadata in JSON files in the archive.',
    ...
    def download(self, params):
        ...
        file containing the listed resource's contents, filtered by permissions.
        """
        user = self.getCurrentUser()
        ...
        cherrypy.response.headers['Content-Disposition'] = 'attachment; filename="Resources.zip"'
            ...
            for kind in resources:
                ...
                for id in resources[kind]:
                    ...
                    for path, file in model.fileList(
                            doc=doc, user=user, includeMetadata=metadata, subpath=True):
    ...
    def _prepareMoveOrCopy(self, params):
        ...
        if 'item' in resources and len(resources['item']) > 0 and parentType != 'folder':
            raise RestException('Invalid parentType.')
    ...
    @describeRoute(
        ...
        .param('resources', 'A JSON-encoded list of types to move.  Each type '
               ...
               'For example: {"item": [(item id 1), (item id2)], "folder": [(folder id 1)]}.')
    ...
    def moveResources(self, params):
        ...
        collection.  Only folder and item resources can be moved with this function.
        """
        user, resources, parent, parentType, progress = self._prepareMoveOrCopy(params)
        total = sum([len(resources[key]) for key in resources])
    ...
    def copyResources(self, params):
        ...
        collection.  Only folder and item resources can be copied with this function.
        """
        user, resources, parent, parentType, progress = self._prepareMoveOrCopy(params)
        ...
        with ProgressContext(progress, user=user, title='Copying resources',
            ...
            for kind in resources:
                ...
                for id in resources[kind]:
                    doc = model.load(id=id, user=user, level=AccessType.READ)
                    if not doc:
                        raise RestException('Resource not found. No %s with id %s' % (kind, id))
