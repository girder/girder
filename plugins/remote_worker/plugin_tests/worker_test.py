...
from girder_remote_worker import celery
from girder_remote_worker.constants import PluginSettings
from girder_remote_worker import utils
from girder_remote_worker.status import CustomJobStatus


def setUpModule():
    base.enabledPlugins.append('remote_worker')
    base.startServer()
...
class WorkerTestCase(base.TestCase):
    ...
    def testWorkerCancel(self):
        ...
        # Schedule the job, make sure it is sent to celery
        with mock.patch('celery.Celery') as celeryMock, \
                mock.patch('girder_remote_worker.event_handlers.AsyncResult') as asyncResult:
            instance = celeryMock.return_value
            instance.send_task.return_value = FakeAsyncResult()
