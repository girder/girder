...
_logger = logging.getLogger('girder.client')
...
class GirderClient(object):
    ...
    def toDict(self):
        """
        Return a serializable version of the girder client instance.

        If this client is authenticated, the dictionary returned by this function will contain
        the authentication token and the deserialized client will have the same permissions
        as the original instance.
        """
        return {
            '__girder_client__': __version__,
            'apiUrl': self.urlBase,
            'token': self.token
        }
    ...
    def fromDict(cls, data):
        """
        Generate a GirderClient instance from a serializable object.

        :param data: A dictionary generated by :py:func:`.toDict`
        :return: A GirderClient instance
        """
        if '__girder_client__' not in data:
            raise ValueError('The value provided was not a girder client serialization')

        version = LooseVersion(data['__girder_client__'])
        if version > LooseVersion(__version__):
            # warn about possible compatibility problems
            _logger.warn('The serialization was generated from a newer version of girder-client')

        client = cls(apiUrl=data['apiUrl'])
...
class JSONEncoder(json.JSONEncoder):
    """
    This class defines a custom json encoder for use with python's ``json`` module that
    handles GirderClient instances natively.

    >>> serialized = json.dumps(GirderClient(), cls=JSONEncoder)
    """
    def default(self, o):
        if isinstance(o, GirderClient):
            return o.toDict()
...
def object_hook(data):
    """
    Provide GirderClient deserialization for python's json module.

    >>> client = json.loads(data, object_hook=object_hook)
    """
    if '__girder_client__' in data:
        return GirderClient.fromDict(data)
    return data
